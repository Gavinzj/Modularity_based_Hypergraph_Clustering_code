...
public class PIC_prune12_speed {
	...
	public PIC_prune12_speed(int trial, String ordering, double ratio,
...
//			deltaQ_short = (incident_weights[clusterIdx] / totalEdgeWeight) + eta_short;
	...
	public void move_split_prune12_largeGraph(int thisNode) {
		...
		// adjacent list of the node
		int secondIdx_ADJ;
		int firstIdx_ADJ = largeADJ_head[split][thisNode];
		...
		if (iteration > 1) {
			if ((curTimeStamp - lastChangeTimes[thisCluster]) > n) {
				...
				clusterIdxs[thisCluster] = distinctNeighborClusterNum++;
				// add cluster of adjacent neighbors
				for (int i = firstIdx_ADJ; i < secondIdx_ADJ; i++) {
...
//			deltaQ_short = (incident_weights[clusterIdx] / totalEdgeWeight) + eta_short;
	...
	public String pic() throws Exception {
		...
		System.out.println("PIC_prune12_speed " + dataset + " " + lambda + " " + gamma);
		...
		do {
			...
			while (hasIncrease) {
				...
				for (int idx = 0; idx < nodeInOrder.length; idx++) {
					...
					// move
					move_split_prune12(nodeInOrder[idx]);		// uncomment this for normal-sized hypergraph
//					move_split_prune12_largeGraph(nodeInOrder[idx]);	// uncomment this for very large hypergraph
