...
public class PIC_noPrune_quality {
	...
	// PIC algorithm with no optimization technique
	public PIC_noPrune_quality(int trial, String ordering, double ratio,
	...
	public void initPart1() {
		...
		// 1) copying the hypergraph in a form of incident list and,
	...
	public String move_split(int thisNode) {
		...
		double delta_eta_cNou = expect(vol_cNou);
		double delta_eta_u = expect(thisDegree);
		double delta_eta_c = expect(vol_c);
		...
		for (int i = 0; i < possibleNeighborClusterNum; i++) {
			...
			delta_eta_c = expect(vol_c);
			delta_eta_u = expect(thisDegree);
			delta_eta_cu = expect(vol_cu);
	...
	private double expect(double vol_c) {
		...
		return  ((1.0 - eta_c) * (1.0 - eta_c)) * (1.0 - gamma) / (1.0 - gamma + (gamma * eta_c));
	}
	...
	public String pic() throws Exception {
		...
		System.out.println("PIC_noPrune_quality " + dataset + " " + lambda + " " + gamma);
